...
get_read_coverages <- function(bam_data, output) {
  ...
  write.csv(bam_export, file = paste0(output, "data.csv"), quote=F, row.names=F) 
...
get_alignment_data_and_summarise <- function(bam_data, output) {
  ...
  write.csv(bam_per_unique_transcript, file = paste0(output, "transcript_level_data.csv"), quote=F, row.names=F)  
  ...
  write.table(stats, file = paste0(output,"stats.csv"), sep=",", quote=F, col.names = FALSE, row.names = FALSE) 
...
plot_coverage <- function(bam_primary, output) {
    if (!has_rows(bam_primary)) {
    message("Skipping coverage histogram – no rows.")
    return(invisible(NULL))
  }
  bam_primary <- bam_primary %>% 
    dplyr::mutate(above=read_coverage>0.95)
  
  pdf(paste0(output, "coverage_fraction.pdf"), width=6, height=6)
...
plot_coverage_vs_length <- function(bam_primary, output) {
  ...
  pdf(paste0(output, "density.pdf"), width=8, height=5)
  plot <- ggplot() +
...
plot_secondary_alignments <- function(alignments, output) {
    if (!has_rows(alignments)) {
    message("Skipping coverage histogram – no rows.")
    return(invisible(NULL))
  }
  pdf(paste0(output, "sec_alns.pdf"), width=8, height=5)
...
plot_transcript_length_distribution <- function(bam_primary, output) {
  ...
  pdf(paste0(output, "transcript_length_distribution_per_distinct_transcript.pdf"), width=6, height=6)
  ...
  pdf(paste0(output, "transcript_length_distribution_per_read.pdf"), width=6, height=6)
...
plot_accuracy <- function(bam_primary, output) {
  ...
  pdf(paste0(output, "read_accuracy.pdf"), width=6, height=6)
...
main <- function() {
  ...
  if (!is.null(output_done) && nzchar(output_done)) {
    writeLines("done", con = output_done)
  }
}
